...
export function parseCSV(text: string): ValidationResult {
  ...
  if (lines.length < 2) {
    return { rows: [], issues: [{ type: "missing", row: 0, message: "Empty file" }], totalRows: 0, validRows: 0 };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (nums.spend < 0) issues.push({ type: "negative_spend", row: i + 1, message: "Negative spend" });
