    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/70 backdrop-blur-xl">
      <div className="container mx-auto flex h-16 items-center justify-between px-6">
        ...
        <nav className="hidden items-center gap-8 text-sm text-muted-foreground md:flex">
          ...
          <a href="#forecasting" className="hover:text-foreground">Forecasting</a>
      ...
      <div className="container mx-auto grid gap-12 px-6 py-24 md:py-32 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <p className="mt-6 max-w-xl text-lg text-muted-foreground">
            ...
            revenue and blended ROAS for the next 30, 60 or 90 days — and tells you exactly where
            to spend next.
          </p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 text-center text-xs">
              {[
                ...
                <div key={c.l} className="rounded-lg border border-border/60 bg-card/40 p-3">
                  ...
                  <div className="mt-1 text-base font-semibold" style={{ color: c.c }}>{c.v}</div>
                </div>
              ))}
            </div>
          </div>
...
function MiniChart() {
  const pts = Array.from({ length: 40 }, (_, i) =>
    50 + 20 * Math.sin(i / 4) + i * 1.2 + (i > 25 ? (i - 25) * 2.5 : 0),
  );
  ...
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="mt-4 h-32 w-full">
      ...
      <path d={path.replace(/M 0 [\d.]+/, (m) => m).replace(/L (\d+\.?\d*) /g, (m, x) => (Number(x) > 65 ? `L ${x} ` : m))}
        stroke="oklch(0.78 0.18 195)" strokeWidth="1.2" strokeDasharray="2 2" fill="none" />
    </svg>
  );
}
...
function Features() {
  const items = [
    { icon: BarChart3, title: "Executive dashboard", desc: "Revenue, spend, ROAS and campaign KPIs with trend charts and channel breakdowns." },
    { icon: Upload, title: "CSV upload & validation", desc: "Drag-and-drop ingestion with duplicate, missing-value and invalid-date detection." },
    { icon: LineChart, title: "Revenue & ROAS forecasts", desc: "30 / 60 / 90 day forecasts at overall, channel, campaign-type or campaign level." },
    { icon: Calculator, title: "Budget simulator", desc: "Slide budgets across Google, Meta and Microsoft Ads — see revenue & ROAS update live." },
    { icon: Brain, title: "AI executive insights", desc: "Drivers, risks, opportunities and budget allocation recommendations written for stakeholders." },
    { icon: Database, title: "Instant demo mode", desc: "365 days of realistic multi-channel data, ready for you to explore in one click." },
  ];
  return (
    <section id="features" className="border-t border-border/60 py-24">
      <div className="container mx-auto px-6">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {items.map((i) => (
            <div key={i.title} className="group rounded-2xl border border-border/60 bg-gradient-card p-6 transition hover:shadow-elevated">
  ...
  return (
    <section id="forecasting" className="border-t border-border/60 bg-muted/30 py-24">
      <div className="container mx-auto grid gap-12 px-6 lg:grid-cols-2 lg:items-center">
        <div>
          <SectionHeader eyebrow="Forecasting" title="Predict the next 90 days with confidence" align="left" />
...
function Benefits() {
  const items = [
    { title: "Plan budgets with data", desc: "Replace gut-feel media planning with statistical forecasts and AI commentary." },
    { title: "Spot risk before it hurts", desc: "Catch underperforming campaigns and channel decay in your weekly review." },
    { title: "Win the QBR", desc: "Export-ready insights, projections and recommendations for the leadership team." },
  ];
  return (
      ...
      <div className="container mx-auto grid gap-12 px-6 lg:grid-cols-2 lg:items-center">
        <div className="rounded-2xl border border-border/60 bg-gradient-card p-6 shadow-elevated">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Revenue is trending +18% QoQ driven by Performance Max and branded search. Meta retargeting
            ROAS slipped to 1.9x — reallocating 15% of its spend to Google PMax projects an additional
            $142K in the next 30 days.
          </p>
          <div className="mt-4 grid gap-2 text-xs">
            {[
              ...
              <div key={l} className="rounded-md bg-card/40 px-3 py-2">{l}</div>
            ))}
          </div>
        </div>
        <div>
          <SectionHeader eyebrow="AI Insights" title="A virtual analyst on your team" align="left" />
          ...
          <Link to="/app/insights" className="mt-6 inline-block">
            <Button variant="hero">Generate insights <Sparkles className="ml-1 h-4 w-4" /></Button>
          </Link>
        </div>
      </div>
    </section>
...
function SectionHeader({ eyebrow, title, align = "center" }: { eyebrow: string; title: string; align?: "left" | "center" }) {
  return (
    <div className={align === "center" ? "mx-auto max-w-2xl text-center" : ""}>
      <div className="text-xs font-medium uppercase tracking-wider text-primary-glow">{eyebrow}</div>
