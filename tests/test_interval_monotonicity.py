...
from backend.backtest import _interval_calibration_snapshot, write_report_files
...
def test_interval_calibration_report_matches_source_constants_and_backtest_summary():
    ...
    ablation = Path("reports/long_horizon_revenue_ablation.md").read_text(encoding="utf-8")
    model_card = Path("reports/model_card.md").read_text(encoding="utf-8")
    ...
    assert calibration["latest_walk_forward_source"] == "reports/backtest_report.json generated by backend.backtest"
    assert calibration["backtest_report_generated_at"] == backtest["generated_at"]
    assert calibration["horizon_planning_selection"] == backtest["horizon_planning_selection"]
    for item in backtest["horizon_planning_selection"]:
        assert f'| {item["horizon_days"]} | {item["selected_method"]} |' in summary
        assert f'| {item["horizon_days"]} | {item["selected_method"]} |' in model_card
        if int(item["horizon_days"]) in {60, 90}:
            assert f'| {item["horizon_days"]}d |' in ablation


def test_backtest_writer_keeps_interval_snapshot_in_lockstep(tmp_path):
    """write_report_files should synchronize all report artifacts from one result object."""
    backtest = json.loads(Path("reports/backtest_report.json").read_text(encoding="utf-8"))
    json_path = tmp_path / "backtest_report.json"
    md_path = tmp_path / "backtest_summary.md"

    write_report_files(backtest, json_path, md_path)
    first_interval = json.loads((tmp_path / "interval_calibration_report.json").read_text(encoding="utf-8"))
    write_report_files(backtest, json_path, md_path)
    second_interval = json.loads((tmp_path / "interval_calibration_report.json").read_text(encoding="utf-8"))
    written_backtest = json.loads(json_path.read_text(encoding="utf-8"))
    written_summary = md_path.read_text(encoding="utf-8")
    written_ablation = (tmp_path / "long_horizon_revenue_ablation.md").read_text(encoding="utf-8")
    written_model_card = (tmp_path / "model_card.md").read_text(encoding="utf-8")

    assert first_interval == second_interval
    assert first_interval["latest_walk_forward_backtest"] == _interval_calibration_snapshot(written_backtest)
    assert first_interval["horizon_planning_selection"] == written_backtest["horizon_planning_selection"]
    for item in written_backtest["per_horizon_performance"]:
        horizon = int(item["horizon_days"])
        coverage = float(item["trained_model_metrics"]["interval_coverage"])
        assert f"| {horizon} |" in written_summary
        assert f"| {coverage}%" in written_summary
    for item in written_backtest["horizon_planning_selection"]:
        assert f'| {item["horizon_days"]} | {item["selected_method"]} |' in written_summary
        assert f'| {item["horizon_days"]} | {item["selected_method"]} |' in written_model_card
        if int(item["horizon_days"]) in {60, 90}:
            assert f'| {item["horizon_days"]}d |' in written_ablation
