...
The offline evaluator path is intentionally isolated: it uses `run.sh`, a compact sklearn model artifact, and no servers, Gemini, frontend, or internet. Per the Hackathon Submission Guide Section 8 no-network runtime rule for submitted evaluators, `run.sh` never calls an LLM; `output/causal_summary.txt` is generated by deterministic causal templates and observational DiD evidence.
...
cross-read duplicate documentation.
