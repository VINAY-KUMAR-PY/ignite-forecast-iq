...
reported lower/upper bands are treated as the evaluator-facing 90% planning
...
|---:|---:|---:|---:|---:|---:|
| 30 days | 2.23% | 1.22% | 100.0% | 100.0% | 14.75% |
| 60 days | 9.54% | 1.11% | 100.0% | 100.0% | 51.76% |
| 90 days | 7.89% | 2.04% | 100.0% | 100.0% | 57.08% |
...
Confidence intervals now use a split-conformal revenue calibration wrapper
around the committed point model. `scripts/calibrate_intervals.py` scores two
chronological calibration windows immediately before the final holdout, derives
the 90% conformal absolute-residual quantile per horizon, and writes the
resulting constants to `backend/evaluator_intervals.py`. The final holdout used
in `reports/backtest_summary.md` is not used for calibration.

| Horizon | Interval Multiplier | Floor (% of expected) | Confidence Z |
|---|---|---|---|
| 30 days | 1.00 | 5.51% | 0.95 |
...
use split-conformal residual calibration, but ROAS bounds are centered on
...
- Committed sample intervals are split-conformal and differentiated: overall
  30/60/90-day widths are 11.08%, 43.36%, and 45.36%; row-level widths range
  from 11.08% to 78.92%.
...
|---:|---:|---:|---:|---:|---:|
| 30 days | 3 | 2.23% | 3.15% | 100.0% | 14.75% |
| 60 days | 3 | 9.54% | 9.54% | 100.0% | 51.76% |
| 90 days | 2 | 7.89% | 7.89% | 100.0% | 57.08% |
...
overconfident relative to the 90% split-conformal target used during
calibration. Widths widen by horizon and by thinner segment grain: trained
revenue width is 9.9-16.34% at 30 days, 43.36-50.78% at 60 days, and
44.36-64.15% at 90 days. The calibration is now an actual chronological
split-conformal wrapper rather than hand-tuned floor narrowing. Production use
should recalibrate bands on larger merchant-specific holdouts to tighten
sharpness further while preserving coverage.
