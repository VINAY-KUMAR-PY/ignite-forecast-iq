...
function forecastTarget(
  ...
  const variance =
    residuals.reduce((s, v) => s + (v - mean) ** 2, 0) / Math.max(1, residuals.length - 1);
...
export function simulateChannelForecast(
  ...
  const baselineDailySpend =
    baseSlice.reduce((s, d) => s + d.spend, 0) / Math.max(1, baseSlice.length);
  const baselineDailyRevenue =
    baseSlice.reduce((s, d) => s + d.revenue, 0) / Math.max(1, baseSlice.length);
