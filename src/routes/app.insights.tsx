      ...
      {!insights && !loading && (
        <Card className="bg-gradient-card border-border/60 p-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Click <em>Generate insights</em> to produce a CMO-level briefing with revenue drivers, channel &amp; campaign analysis, budget allocation, risks and growth opportunities.
          </p>
        </Card>
      )}
      ...
      {loading && (
        <Card className="bg-gradient-card border-border/60 p-10 text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            Analyzing {rows.length.toLocaleString()} rows across {summary.channels.length} channels and {summary.totalCampaigns} campaigns…
          </p>
        </Card>
      )}
                ...
                <ul className="space-y-2">
                  {insights.campaignPerformance?.bottom?.map((c, i) => (
                    <li key={i} className="rounded-md border border-border/40 bg-background/40 p-3">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground"><span className="font-medium text-destructive">Issue: </span>{c.issue}</p>
                      <p className="mt-1 text-xs text-muted-foreground"><span className="font-medium text-foreground">Action: </span>{c.action}</p>
                    </li>
                  ))}
                </ul>
              </div>
                ...
                return (
                  <div key={i} className="rounded-lg border border-border/40 bg-background/40 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {fmtPct(b.currentSharePct / 100)} → <span className="font-medium text-foreground">{fmtPct(b.recommendedSharePct / 100)}</span>
                        </span>
                      </div>
                      <span
                    ...
                    <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-muted">
                      <div className="h-full bg-primary" style={{ width: `${Math.min(100, Math.max(0, b.recommendedSharePct))}%` }} />
                    </div>
                    <p className="mt-3 text-xs text-muted-foreground">{b.rationale}</p>
                    <p className="mt-1 text-xs"><span className="font-medium text-success">Expected impact: </span><span className="text-muted-foreground">{b.expectedImpact}</span></p>
                  </div>
                );
              })}
            </div>
...
function buildSummary(rows: CampaignRow[]) {
  ...
  const cMap = new Map<string, { revenue: number; spend: number; conversions: number; channel: string; type: string }>();
  for (const r of rows) {
    const k = `${r.channel}|${r.campaign_name}`;
    const c = cMap.get(k) ?? { revenue: 0, spend: 0, conversions: 0, channel: r.channel, type: r.campaign_type };
    c.revenue += r.revenue;
    c.spend += r.spend;
  ...
  const roasTrendPct = prev30Roas > 0 ? Math.round(((last30Roas - prev30Roas) / prev30Roas) * 1000) / 10 : 0;
    ...
    <Card className="bg-gradient-card border-border/60 p-4">
      ...
      <div className="mt-1 flex items-baseline gap-2">
        ...
        {typeof delta === "number" && delta !== 0 && (
          <span className={`text-[11px] font-medium ${delta > 0 ? "text-success" : "text-destructive"}`}>
...
function VerdictBadge({ verdict }: { verdict: "outperforming" | "on_track" | "underperforming" }) {
  const map = {
    ...
    underperforming: { cls: "bg-destructive/15 text-destructive border-destructive/30", label: "Underperforming" },
  } as const;
...
function SeverityBadge({ severity }: { severity: "low" | "medium" | "high" }) {
  ...
  return <Badge variant="outline" className={`text-[10px] ${s.cls}`}>{s.label} risk</Badge>;
}
