...
function Dashboard() {
  ...
  if (!stats) return (<><PageHeader title="Dashboard" /><EmptyState /></>);

  // Sample daily for chart (every Nth point if long)
  const dailySample = stats.daily.length > 120 ? stats.daily.filter((_, i) => i % 3 === 0 || i === stats.daily.length - 1) : stats.daily;
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Revenue" value={fmtCurrency(stats.totalRevenue)} delta={stats.revDelta} icon={DollarSign} hint="vs prior 30d" />
        <KpiCard label="Total Spend" value={fmtCurrency(stats.totalSpend)} delta={stats.spendDelta} icon={Activity} hint="vs prior 30d" />
        <KpiCard label="Avg ROAS" value={fmtRoas(stats.avgRoas)} delta={stats.roasDelta} icon={Target} hint="vs prior 30d" />
        <KpiCard label="Campaigns" value={stats.campaigns.toString()} icon={TrendingUp} hint={`${stats.channels.length} channels`} />
      </div>
        ...
        <ChartCard title="Revenue trend" subtitle="Daily revenue over time">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={dailySample} margin={{ left: -10, right: 8, top: 8 }}>
              ...
              <XAxis dataKey="date" tickFormatter={fmtDate} stroke="var(--color-muted-foreground)" fontSize={11} minTickGap={40} />
        ...
        <ChartCard title="Spend trend" subtitle="Daily media spend">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={dailySample} margin={{ left: -10, right: 8, top: 8 }}>
              ...
              <YAxis stroke="var(--color-muted-foreground)" fontSize={11} tickFormatter={(v) => fmtCompact(v as number)} />
              <Tooltip content={<TT formatter={fmtCurrency} />} />
              <Area type="monotone" dataKey="spend" stroke="var(--color-chart-2)" fill="url(#sp)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="ROAS trend" subtitle="Blended ROAS by day">
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={dailySample} margin={{ left: -10, right: 8, top: 8 }}>
              ...
              <Line type="monotone" dataKey="roas" stroke="var(--color-chart-3)" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
