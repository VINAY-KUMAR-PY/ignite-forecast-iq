...
ForecastIQ uses rolling-origin evidence to select the planning method per
horizon. The 30-day revenue forecast uses the trained residual-correction
model because paired validation favors it. The 60-day and 90-day revenue
forecasts are labeled `trained_model_baseline_anchored` because the trained
artifact is available but the seasonal baseline is safer for revenue planning
at those horizons. This is intentional: the CSV carries the recommended
planning forecast, while the reports preserve the trained-vs-baseline evidence.

During earlier hardening, the training code was extended with additional
...
current champion-challenger policy does not force those signals to win; it
uses them only when rolling-origin validation supports them.
...
|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|---|
| 60d | 19199.67 | 19199.67 | 30397.05 | 30397.05 | 10.11% | 10.11% | champion-challenger selected baseline anchor | scorer-applied baseline anchor | n/a | baseline anchor |
| 90d | 22141.94 | 22141.94 | 34041.40 | 34041.40 | 7.89% | 7.89% | champion-challenger selected baseline anchor | scorer-applied baseline anchor | n/a | baseline anchor |

The selected 60-day and 90-day rows remain artifact-backed and schema-valid,
but they use the baseline planning point estimate because the current
rolling-origin evidence does not justify extrapolating the trained residual
signal. `safe_baseline_fallback` remains reserved for runtime degradation cases
such as missing/corrupt model files, empty input, malformed schemas, or
...
| Horizon | Revenue MAPE | ROAS MAPE | Revenue interval coverage |
|---:|---:|---:|---:|
| 30d | 2.81% | 1.26% | 95.83% |
| 60d | 10.11% | 1.56% | 90.28% |
| 90d | 7.89% | 2.46% | 86.11% |
...
campaign mix changes. The 60-day interval floor was recalibrated from the
four-window rolling-origin residual ratios after the multi-horizon point model
changed; the final report now lands inside the 85-95% planning band without
changing point forecasts. ForecastIQ reports mean interval width beside coverage
...
The current 90-day coverage is 86.11%, inside the practical planning band but
lower than 30/60 days because only two non-overlapping 90-day rolling windows
are available in the sample data.
...
| `trained_model_baseline_anchored` | Rolling-origin evidence favors the seasonal baseline for a supported 60/90-day revenue planning row | Transparent trained-artifact variant; emitted by the current committed 60/90-day sample output. |
