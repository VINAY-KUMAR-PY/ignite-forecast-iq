...
const CHANNELS: { name: Channel; types: string[]; campaigns: string[]; baseRoas: number; baseSpend: number }[] = [
  {
    name: "Google Ads",
    types: ["Search", "Shopping", "Performance Max"],
    campaigns: ["Brand Search", "Generic Search", "Shopping - Best Sellers", "PMax - Holiday", "Competitor"],
    baseRoas: 4.2,
    baseSpend: 1800,
  },
  {
    ...
    campaigns: ["Prospecting - Lookalike", "Retargeting - Cart", "Advantage+ Shopping", "Engagement", "Reels Ads"],
    baseRoas: 3.1,
    baseSpend: 1500,
  },
  {
  ...
  for (let d = days - 1; d >= 0; d--) {
    ...
    for (const ch of CHANNELS) {
      for (const campaign of ch.campaigns) {
        ...
        const roas = Math.round(ch.baseRoas * roasNoise * (campaign.includes("Brand") ? 1.4 : 1) * 100) / 100;
