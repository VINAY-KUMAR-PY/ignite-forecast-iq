...
| Rolling samples      | 414                          |
| Feature count        | 35                           |
| Revenue blend weight | 0.35 per horizon             |
...
The trained evaluator now contributes a nonzero revenue model blend for every horizon. On the primary 30-day sample holdout, the safe baseline still wins revenue point MAE, while the trained path preserves 100% revenue interval coverage and improves ROAS RMSE/MAPE. This is reported honestly rather than presented as artificial lift. The summary is generated by:
...
| --- | ---: | ---: | ---: | --- |
| Revenue | 2,649.77 | 2,185.89 | Trained MAE +21.22% | Safe baseline |
| ROAS | 0.05 | 0.05 | Tie on rounded MAE | Tie |

Plain-English interpretation: revenue point estimates remain guarded by the deterministic baseline because the sample holdout shows baseline revenue MAE is lower. The trained artifact still contributes a CV-gated 0.35 revenue blend, ROAS behavior, and uncertainty calibration. ForecastIQ therefore does not overclaim trained-model dominance.
...
Blend-weight validation tested revenue model weights of 0.00, 0.10, 0.25, 0.40, 0.50, and 0.60. The packaged artifact uses a CV-gated 0.35 blend so the trained model is not ignored; the report still shows that the deterministic baseline remains the lower-error revenue benchmark on this sample. ROAS uses 0.40 because that weight produced the best ROAS RMSE/MAE balance.
...
| ------: | ----: | -------: | ------------------: | -------------------: | ---------------: | ---------------: | ------------------ |
|      30 |     3 |       54 |            3,780.22 |             3,097.88 |             0.05 |           98.15% | Safe baseline |
|      60 |     3 |       54 |           19,891.14 |            11,221.15 |             0.12 |           88.89% | Safe baseline |
