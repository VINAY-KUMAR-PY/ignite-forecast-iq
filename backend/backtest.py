...
def _metrics(rows: list[dict[str, Any]], prefix: str) -> dict[str, float]:
    ...
    coverage = [
        _safe_float(row[f"{prefix}_lower_revenue"]) <= _safe_float(row["actual_revenue"]) <= _safe_float(row[f"{prefix}_upper_revenue"])
        for row in rows
    ]
    return {
...
def run_backtest(data_dir: str | Path = "data", holdout_days: int = 30) -> dict[str, Any]:
    """Train on all but the final holdout period and score the 30-day forecast."""
    ...
    if train_frame.empty or test_frame.empty:
        raise SystemExit("Backtest split produced empty train or test data.")
    ...
    for level, segment, history in segment_specs(train_frame):
        ...
        try:
            trained = trained_forecast_segment(history, holdout_days, level, segment, trained_model)
            trained_model_type = TRAINED_MODEL_TYPE
        except Exception:
            trained = forecast_segment(history, holdout_days, baseline)
            trained_model_type = SAFE_BASELINE_MODEL_TYPE

        safe = forecast_segment(history, holdout_days, baseline)
        rows.append(
            {
                "level": level,
                "segment": segment,
...
- Training period: all valid sample rows before the final {report["holdout_days"]} days
- Test period: final {report["holdout_days"]} days
...
| --- | ---: | ---: | ---: | ---: |
| Trained model | {trained["mae"]} | {trained["rmse"]} | {trained["mape"]}% | {trained["interval_coverage"]}% |
| Safe baseline | {safe["mae"]} | {safe["rmse"]} | {safe["mape"]}% | {safe["interval_coverage"]}% |
