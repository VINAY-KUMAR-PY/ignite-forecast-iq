...
|---|---|---|
| Hierarchical reconciliation candidate | Compare segment-level residual forecasts against roll-up consistency with channel/campaign-type totals. | Not adopted because the paired bootstrap gate still did not show a p < 0.05 long-horizon win over the seasonal baseline. |
| Channel/campaign-type mix drift | Detect whether spend share movement supports residual correction at 60/90 days. | Reviewed as an additional feature family; evidence remained a statistical tie in the committed rolling-origin windows. |
| Spend elasticity and ROAS trend terms | Let the residual model react to sustained efficiency or budget-response changes. | Retained as documented candidate evidence, but the evaluator artifact remains baseline-anchored where the model does not beat the deterministic baseline. |
